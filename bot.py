...
class YouTubeAPIUnavailable(Exception):
    """Raised when the YouTube API can't give us a reliable answer
    (quota exceeded, rate limited, network/timeout, 5xx, etc).
    This is NOT the same as 'video/channel actually removed' and must
    never be treated as a removal signal."""
    pass
...
def is_admin_or_owner():
    ...
    async def predicate(interaction: discord.Interaction) -> bool:
        ...
        # Check if user has specific admin roles (optional - can be customized)
        admin_roles = ["Admin", "Administrator", "Owner", "Moderator"]
        user_roles = [role.name for role in interaction.user.roles]
        
        for admin_role in admin_roles:
            if admin_role.lower() in [r.lower() for r in user_roles]:
                return True
        
        await interaction.response.send_message(
            "❌ Only **Server Owners** and **Admins** can use this command.", 
            ephemeral=True
        )
        return False
...
# ============================================================
# YOUTUBE API FUNCTIONS (FULLY FIXED)
# ============================================================
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url, params=params, timeout=10) as resp:
                if resp.status == 200:
                    ...
                    # 200 OK but no matching item -> genuinely not found/removed
                    print(f"Channel {channel_id} not found (empty result, may be banned/deleted)")
                    return None
                elif resp.status == 404:
                    print(f"Channel {channel_id} not found (may be banned/deleted)")
                    return None
                elif resp.status == 403:
                    print(f"⚠️ YouTube API quota exceeded (channel check)")
                    raise YouTubeAPIUnavailable("quota exceeded")
                else:
                    print(f"⚠️ YouTube API error: {resp.status} (channel check)")
                    raise YouTubeAPIUnavailable(f"HTTP {resp.status}")
    except YouTubeAPIUnavailable:
        raise
    except Exception as e:
        print(f"⚠️ Channel info request failed (network/timeout): {e}")
        raise YouTubeAPIUnavailable(str(e))
...
async def get_channel_info_by_handle(handle: str) -> Optional[Dict]:
    ...
    # METHOD 1: Direct channel lookup with forHandle
    try:
            ...
            async with session.get(channel_url, params=channel_params, timeout=10) as resp:
                if resp.status == 200:
                    ...
                    if data.get("items"):
                        ...
                        print(f"✅ Found channel via forHandle: {channel_id}")
                        return {
                            "channel_id": channel_id,
                            "channel_name": snippet.get("title", "Unknown"),
                            "channel_url": f"https://youtube.com/channel/{channel_id}",
        ...
        print(f"forHandle method error: {e}")
            ...
            async with session.get(search_url, params=search_params, timeout=10) as resp:
                if resp.status == 200:
                    ...
                    if data.get("items"):
                        ...
                        if channel_id:
                            print(f"✅ Found channel via search: {channel_id}")
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url, params=params, timeout=10) as resp:
                if resp.status == 200:
                    ...
                    if data.get("items"):
                        ...
                        if channel_id:
                            try:
                                ...
                                # Supplementary info only — don't fail the whole
                                # video check just because this sub-lookup hit quota.
                                print("⚠️ Could not fetch channel info (quota/API issue), continuing without it")
                    ...
                    # 200 OK but no matching item -> genuinely not found/removed
                    print(f"Video {video_id} not found (empty result, likely removed)")
                    return None
                elif resp.status == 404:
                    print(f"Video {video_id} not found (may be removed)")
                    return None
                elif resp.status == 403:
                    print(f"⚠️ YouTube API quota exceeded (video check)")
                    raise YouTubeAPIUnavailable("quota exceeded")
                else:
                    print(f"⚠️ YouTube API error: {resp.status} (video check)")
                    raise YouTubeAPIUnavailable(f"HTTP {resp.status}")
    except YouTubeAPIUnavailable:
        raise
    except Exception as e:
        print(f"⚠️ Video info request failed (network/timeout): {e}")
        raise YouTubeAPIUnavailable(str(e))
...
async def check_tracked_videos():
    ...
    try:
        ...
        for video_id, meta in tracked.items():
            ...
            except YouTubeAPIUnavailable as e:
                print(f"⏭️ Skipping video {video_id} this cycle (API unavailable: {e})")
                continue
    ...
    try:
        ...
        for channel_id, meta in tracked.items():
            ...
            try:
                ...
                print(f"⏭️ Skipping channel {channel_id} this cycle (API unavailable: {e})")
                continue
...
# ============================================================
# DISCORD COMMANDS (ALL WITH ADMIN/OWNER PERMISSION)
# ============================================================
...
async def on_ready():
    try:
        ...
        print(f"✅ Only Server Owners and Admins can use commands")
