...
FIXED: Channel tracking with @handles working properly.
"""
...
async def get_channel_info_by_handle(handle: str) -> Optional[Dict]:
    """Get channel info by @handle - FULLY FIXED VERSION"""
    if not YOUTUBE_API_KEY:
        return None
    
    # Clean the handle - remove @ and anything after ? or &
    handle = handle.lstrip('@').split('?')[0].split('&')[0]
    print(f"🔍 Searching for channel with handle: {handle}")
    
    # METHOD 1: Direct channel lookup with forHandle parameter
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(channel_url, params=channel_params, timeout=10) as resp:
                if resp.status == 200:
                    ...
                    print(f"forHandle method: Channel not found: {handle}")
                else:
                    print(f"forHandle method: API error {resp.status}")
    except Exception as e:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(search_url, params=search_params, timeout=10) as resp:
                if resp.status == 200:
                    ...
                    print(f"Search method: API error {resp.status}")
    except Exception as e:
        print(f"Search method error: {e}")
    
    # METHOD 3: Try with the handle as a username (legacy)
    try:
        username_params = {
            "part": "snippet,statistics",
            "forUsername": handle,
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(channel_url, params=username_params, timeout=10) as resp:
                if resp.status == 200:
                    data = await resp.json()
                    if data.get("items"):
                        item = data["items"][0]
                        ...
                        stats = item.get("statistics", {})
                        channel_id = item.get("id", "")
                        print(f"✅ Found channel via forUsername: {channel_id}")
                        return {
                            "channel_id": channel_id,
                            "channel_name": snippet.get("title", "Unknown"),
                            "channel_url": f"https://youtube.com/channel/{channel_id}",
                    ...
                    print(f"forUsername method: Channel not found: {handle}")
    except Exception as e:
        print(f"forUsername method error: {e}")
...
async def trackchannel(interaction: discord.Interaction, channel_url: str):
    ...
    try:
        ...
        # Get channel info using the found ID
        info = await get_channel_info(channel_id)
        if not info:
        ...
        # Remove if previously tracked (recovered)
