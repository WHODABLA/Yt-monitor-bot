            ...
            async with session.get(url, params=params, timeout=10) as resp:
                if resp.status == 200:
                    ...
                    if data.get("items"):
                        ...
                        # No items found - video might be removed or private
                        return None
                elif resp.status == 403:
                    print(f"⚠️ YouTube API quota exceeded")
                    return None
    ...
    try:
        ...
        for video_id, meta in tracked.items():
            ...
            if info is None:
                ...
                print(f"⚠️ Video returned None - verifying...")
                await asyncio.sleep(2)
                info_retry = await get_video_info(video_id)
                
                if info_retry is None:
                    ...
                    # Second check worked - API error, not removal
                    print(f"✅ Video still available (API error on first check)")
                    await api_update_video_stats(video_id, info_retry)
            else:
                # Video exists - update stats
                await api_update_video_stats(video_id, info)
                print(f"📊 Updated stats: {info.get('title')[:50]}...")
...
async def check_tracked_channels():
    ...
    try:
        ...
        for channel_id, meta in tracked.items():
            ...
            if info is None:
                # Double-check to avoid false positives
                print(f"⚠️ Channel returned None - verifying...")
                await asyncio.sleep(2)
                info_retry = await get_channel_info(channel_id)
                ...
                if info_retry is None:
                    ...
                    # Second check worked
                    print(f"✅ Channel still available (API error on first check)")
                    await api_update_channel_stats(channel_id, info_retry)
            else:
                # Channel exists - update stats
                await api_update_channel_stats(channel_id, info)
                print(f"📊 Updated stats: {info.get('channel_name')[:50]}... - {format_number(info.get('subscribers', 0))} subscribers")
