"""
YouTube Monitor — Discord Bot (VIDEOS + CHANNELS)
--------------------------------------
            ...
            for row in result:
                ...
                if channel_id:
                    tracked[channel_id] = {
                        ...
                        "videos_count": int(row.get("videos_count", 0)),
                        "start_time": row.get("start_time", datetime.now(timezone.utc).isoformat()),
                        "recovered": bool(row.get("recovered", 0)),
                        "recovered_at": row.get("recovered_at"),
                        "last_stats": json.loads(row.get("last_stats", "{}")) if row.get("last_stats") else {},
    ...
    try:
        ...
        for video_id, meta in tracked.items():
            ...
            if info is None:
                ...
                print(f"📊 Updated video stats: {video_id}")
    ...
    try:
        ...
        for channel_id, meta in tracked.items():
            ...
            if info is None:
                ...
                print(f"📊 Updated channel stats: {channel_id}")
...
async def trackchannel(interaction: discord.Interaction, channel_url: str):
    ...
    try:
        ...
        # If no channel ID found, try to get info from URL
        if not channel_id:
            # Try @handle format
            ...
            if handle_match:
                ...
                if info:
                    ...
                    await interaction.followup.send(f"❌ Could not find channel with @{handle}")
                    return
            else:
