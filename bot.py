"""
YouTube Video Monitor — Discord Bot
--------------------------------------
Tracks YouTube videos and sends Discord notifications when they become unavailable.
Shows full stats: title, views, likes, comments, etc.
"""
...
if not D1_WORKER_URL:
    raise SystemExit("❌ D1_WORKER_URL not set in environment variables")
if not D1_API_KEY:
    raise SystemExit("❌ D1_API_KEY not set in environment variables")
if not YOUTUBE_API_KEY:
    print("⚠️ YOUTUBE_API_KEY not set - some features will be limited")
# ===============================================
    ...
    if result and isinstance(result, list):
        return {
            row["video_id"]: {
                ...
                "last_stats": json.loads(row.get("last_stats", "{}")),
            }
            for row in result
        }
    return {}
...
def build_removed_embed(info: Dict, start_iso: str, old_stats: Dict = None) -> discord.Embed:
    ...
    # Add last known stats if available
    if old_stats:
    ...
    try:
        ...
        for video_id, meta in tracked.items():
            ...
            if info is None:
                ...
                print(f"✅ Notified removal: {video_id}")
            else:
                # Video still exists - update stats
                await api_update_stats(video_id, info)
                print(f"📊 Updated stats for {video_id}")
...
async def trackvideo(interaction: discord.Interaction, url: str):
    ...
    if not info:
        ...
        if video_id in tracked:
            info = tracked[video_id].get("last_stats", {})
            if info:
                info["title"] = tracked[video_id].get("title", "Unknown")
                info["channel"] = tracked[video_id].get("channel", "Unknown")
...
async def listvideos(interaction: discord.Interaction):
    ...
    for video_id, meta in tracked.items():
        ...
        lines.append(f"`{video_id}` — {title} [{status}]")
    
    lines.insert(1, f"📊 Active: {active} | Removed: {removed}")
    lines.insert(2, "─" * 30)
...
@bot.tree.command(name="checkvideo", description="Check a video instantly")
@app_commands.describe(video_id_or_url="Video ID or YouTube URL")
@is_owner()
...
@bot.tree.command(name="videostats", description="Get current stats for a video")
@app_commands.describe(video_id_or_url="Video ID or YouTube URL")
@is_owner()
