...
async def check_tracked_videos():
    ...
    try:
        ...
        for video_id, meta in tracked.items():
            ...
            print(f"🔍 Checking video: {meta.get('title', video_id)[:50]}...")
            ...
            if result["status"] == "ok":
                ...
                await api_update_video_stats(video_id, result["data"])
                print(f"📊 Updated stats: {result['data'].get('title')[:50]}...")
                continue
            
            if result["status"] in ("quota", "error"):
                ...
                print(f"⏭️ Skipping {video_id} this cycle - status: {result['status']}")
                continue
            
            # status == "not_found" - double-check before concluding it's really gone
            print(f"⚠️ Video not found - verifying...")
            await asyncio.sleep(5)
            retry_result = await get_video_info_with_status(video_id)
            
            if retry_result["status"] == "ok":
                # Second check worked - transient blip, not removal
                print(f"✅ Video still available (transient issue on first check)")
                await api_update_video_stats(video_id, retry_result["data"])
                continue
            
            if retry_result["status"] in ("quota", "error"):
                # Inconclusive on the retry too - don't treat as removal
                print(f"⏭️ Skipping {video_id} this cycle - retry status: {retry_result['status']}")
                continue
            
            # Both checks confirmed not_found - this is a real removal
...
async def check_tracked_channels():
    ...
    try:
        ...
        for channel_id, meta in tracked.items():
            ...
            print(f"🔍 Checking channel: {meta.get('channel_name', channel_id)[:50]}...")
            
            # First check
            result = await get_channel_info_with_status(channel_id)
            
            if result["status"] == "ok":
                # Channel exists - update stats
                await api_update_channel_stats(channel_id, result["data"])
            ...
            if result["status"] in ("quota", "error"):
                ...
                print(f"⏭️ Skipping {channel_id} this cycle - status: {result['status']}")
                continue
            
            # status == "not_found" - double-check before concluding it's really gone
            print(f"⚠️ Channel not found - verifying...")
            await asyncio.sleep(5)
            retry_result = await get_channel_info_with_status(channel_id)
            
            if retry_result["status"] == "ok":
                # Second check worked - transient blip, not removal
                print(f"✅ Channel still available (transient issue on first check)")
                await api_update_channel_stats(channel_id, retry_result["data"])
                continue
            
            if retry_result["status"] in ("quota", "error"):
                # Inconclusive on the retry too - don't treat as removal
                print(f"⏭️ Skipping {channel_id} this cycle - retry status: {retry_result['status']}")
                continue
            
            # Both checks confirmed not_found - this is a real removal
            # Check cooldown
            current_time = datetime.now(timezone.utc).timestamp()
            if channel_id in NOTIFICATION_COOLDOWN:
                cooldown_time = NOTIFICATION_COOLDOWN[channel_id]
                if current_time - cooldown_time < 3600:  # 1 hour cooldown
                    print(f"⏰ Skipping {channel_id} - cooldown active")
                    continue
            
            print(f"🚨 Channel removed: {meta.get('channel_name', channel_id)[:50]}...")
            ...
            try:
                ...
                NOTIFICATION_COOLDOWN[channel_id] = current_time
                print(f"✅ Notification sent for: {meta.get('channel_name', channel_id)[:50]}...")
            except Exception as e:
                print(f"❌ Failed to send: {e}")
